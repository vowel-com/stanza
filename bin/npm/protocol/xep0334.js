...
function processingHints() {
    return {
        ...
        exporter(xml, value, context) {
            if (value.noCopy) {
                xml.appendChild(
                    jxt_1.createElement(Namespaces_1.NS_HINTS, 'no-copy', context.namespace, xml)
                );
            }
            if (value.noPermanentStore) {
                xml.appendChild(
                    jxt_1.createElement(
                        Namespaces_1.NS_HINTS,
                        'no-permanent-store',
                        context.namespace,
                        xml
                    )
            ...
            if (value.noStore) {
                xml.appendChild(
                    jxt_1.createElement(Namespaces_1.NS_HINTS, 'no-store', context.namespace, xml)
                );
            }
            if (value.store) {
                xml.appendChild(
                    jxt_1.createElement(Namespaces_1.NS_HINTS, 'store', context.namespace, xml)
                );
            }
        }
    };
